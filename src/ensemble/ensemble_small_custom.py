"""
Multi-Model Ensemble Bias Detection System with Collaborative Discussion (Small Models)

This system combines three small LLMs (Llama3.2-3B, Qwen3-4B, Mistral-7B) for political bias detection:
...
# Import existing model classes for small models
...
class ModelAgent:
    ...
    Attributes:
        agent_id: Unique identifier (llama32/qwen3/gemma3)
...
class EnsembleMultiModelDetector:
    ...
    Uses Llama-3.2-3B, Qwen3-4B, and Mistral-7B models sequentially, then applies
    consensus analysis and collaborative discussion for disagreements.
    """
    
    def __init__(self, config: Dict = None, batch_size: int = 6, dataset_type: str = 'baly', 
        ...
        if output_dir:
            ...
            self.output_dir = Path("ensemble_outputs_small") / f"session_{self.session_timestamp}"
            self.output_dir.mkdir(parents=True, exist_ok=True)
        ...
        self.llama32_labeler = LlamaLabeler(
            base_url=small_cfg['llama32']['base_url'],
            model_id=small_cfg['llama32']['model_id'],
            api_key=api_key,
        )
        self.qwen3_labeler = QwenLabeler(
            base_url=small_cfg['qwen3']['base_url'],
            model_id=small_cfg['qwen3']['model_id'],
            api_key=api_key,
            enable_thinking=small_cfg['qwen3']['enable_thinking'],
        )
        self.mistral_labeler = MistralLabeler(
            base_url=small_cfg['mistral']['base_url'],
            model_id=small_cfg['mistral']['model_id'],
            api_key=api_key,
        )
        ...
        logger.info(f"Initialized EnsembleMultiModelDetector (Small Models)")
    ...
    async def process_articles(self, articles: List[tuple]) -> List[Dict]:
        ...
        logger.info(f"Processing {len(articles)} articles through small ensemble system")
        
        # Phase 1: Individual Analysis
        logger.info("=" * 60)
        logger.info("PHASE 1: Individual Model Analysis (Small Models)")
        logger.info("=" * 60)
        
        llama32_results, qwen3_results, mistral_results = self._run_individual_analysis(articles)
        
        # Align results by filename to handle per-model failures and ensure 1:1 correspondence
        llama32_results, qwen3_results, mistral_results = self._align_results_by_filename(
            llama32_results, qwen3_results, mistral_results, articles
        )
        
        # Validate aligned results
        if not self._validate_results(llama32_results, qwen3_results, mistral_results):
            raise ValueError("Model results validation failed")
        
        # Update total articles to the aligned count
        self.stats['total_articles'] = len(llama32_results)
        ...
        initial_results = []
        for i in range(len(llama32_results)):
            consensus = self._check_consensus(llama32_results[i], qwen3_results[i], mistral_results[i], update_stats=False)
            initial_result = {
                'article_id': llama32_results[i].get('article_id', i),
                'filename': llama32_results[i]['filename'],
        ...
        if self.dataset_type != 'custom':
            ...
            logger.info("Skipping pre-discussion evaluation for custom dataset")
            pre_discussion_metrics = None
            self.pre_discussion_metrics = None
        ...
        for i in range(len(llama32_results)):
            llama32_res = llama32_results[i]
            qwen3_res = qwen3_results[i]
            mistral_res = mistral_results[i]
            article_id = llama32_res.get('article_id', i)
            
            # Check consensus (update_stats=True by default)
            consensus = self._check_consensus(llama32_res, qwen3_res, mistral_res)
            
            # Prepare final result
            final_result = {
                'article_id': article_id,
                'filename': llama32_res['filename'],
                'models_used': ['llama32', 'qwen3', 'mistral'],
                'individual_scores': {
                    'llama32': {'score': llama32_res['score'], 'direction': llama32_res['direction']},
                    'qwen3': {'score': qwen3_res['score'], 'direction': qwen3_res['direction']},
            ...
            if consensus['needs_discussion']:
                ...
                score_details = f"llama32:{llama32_res['score']:.0f}, qwen3:{qwen3_res['score']:.0f}, mistral:{mistral_res['score']:.0f}"
                ...
                # Get original article content (handle both 'content' and 'content_original' fields)
                article_data = articles[article_id][0] if article_id < len(articles) else {}
                article_content = article_data.get('content') or article_data.get('content_original', "")
                ...
                try:
                    ...
                    discussion_result = await asyncio.wait_for(
                        self._run_collaborative_discussion(
                            article_content, llama32_res, qwen3_res, mistral_res, article_id=i
                        ),
            ...
            self._save_article_results(article_id, llama32_res, qwen3_res, mistral_res, final_result)
        
        # Save session summary
        self._save_session_summary(final_results)
        
        # Calculate post-discussion metrics (skip for custom datasets)
        if self.dataset_type != 'custom':
            logger.info("=" * 60)
            ...
            logger.info("Skipping post-discussion evaluation for custom dataset")
        
        # Calculate and display comprehensive evaluation (skip for custom datasets)
        if self.dataset_type != 'custom':
            logger.info("=" * 80)
            logger.info("COMPREHENSIVE EVALUATION ANALYSIS (SMALL MODELS)")
            logger.info("=" * 80)
            self._comprehensive_evaluation(final_results, articles)
        else:
            logger.info("Skipping evaluation for custom dataset (no ground truth available)")
        
        logger.info(f"Small ensemble processing completed: {len(final_results)} articles processed")
    ...
    def _run_individual_analysis(self, articles: List[tuple]) -> Tuple[List, List, List]:
        ...
        Returns:
            Tuple of (llama32_results, qwen3_results, mistral_results)
        """
        # Process Llama 3.2 3B
        logger.info("Processing with Llama-3.2-3B-Instruct...")
        llama32_results = self._process_single_model('llama32', articles, self.llama32_labeler)
        self._save_individual_results('llama32', llama32_results, articles)

        # Process Qwen3 4B
        logger.info("Processing with Qwen3-4B...")
        qwen3_results = self._process_single_model('qwen3', articles, self.qwen3_labeler)
        self._save_individual_results('qwen3', qwen3_results, articles)

        # Process Mistral 7B
        logger.info("Processing with Mistral-7B-Instruct-v0.3...")
    ...
    def _check_consensus(self, llama32_res: Dict, qwen3_res: Dict, mistral_res: Dict, update_stats: bool = True) -> Dict:
        """
        ...
        """
        directions = [llama32_res['direction'], qwen3_res['direction'], mistral_res['direction']]
        scores = [llama32_res['score'], qwen3_res['score'], mistral_res['score']]
    ...
    async def _run_collaborative_discussion(self, article_content: str, 
                                           llama32_res: Dict, qwen3_res: Dict, mistral_res: Dict,
        ...
        Args:
            ...
            llama32_res, qwen3_res, mistral_res: Initial analysis from each model
        ...
        try:
            ...
            for model_name, res in [('llama32', llama32_res), ('qwen3', qwen3_res), ('mistral', mistral_res)]:
            ...
            initial_positions = {
                'llama32': {
                    'score': llama32_res['score'],
                    'direction': llama32_res['direction'],
                    'reason': llama32_res.get('reason', '')
                },
                'qwen3': {
                    'score': qwen3_res['score'],
                    'direction': qwen3_res['direction'],
                    'reason': qwen3_res.get('reason', '')
                },
                'mistral': {
            ...
            agents = {
                'llama32': ModelAgent('llama32', llama32_res),
                'qwen3': ModelAgent('qwen3', qwen3_res),
            ...
            logger.info(f"Initial directions: llama32:{initial_positions['llama32']['direction']}, "
                       f"qwen3:{initial_positions['qwen3']['direction']}, "
    ...
    async def _generate_challenge(self, challenger: ModelAgent, target: ModelAgent, 
        ...
        if challenger.agent_id == 'llama32':
            labeler = self.llama32_labeler
        elif challenger.agent_id == 'qwen3':
            labeler = self.qwen3_labeler
        else:
    ...
    async def _generate_response(self, responder: ModelAgent, challenge: str, challenger: ModelAgent,
        ...
        # Get appropriate labeler
        if responder.agent_id == 'llama32':
            labeler = self.llama32_labeler
        elif responder.agent_id == 'qwen3':
            labeler = self.qwen3_labeler
        else:
            labeler = self.mistral_labeler
    ...
    def _save_individual_results(self, model_name: str, results: List[Dict], articles: List[tuple]):
        ...
        # Build filename -> (article_data, idx) map for robust lookup
        filename_to_idx: Dict[str, int] = {}
        filename_to_article: Dict[str, Dict] = {}
        for idx, (article_data, fname) in enumerate(articles):
            filename_to_idx[fname] = idx
            filename_to_article[fname] = article_data
        
        for result in results:
            fname = result.get("filename")
            enhanced_result = result.copy()
            if fname in filename_to_article:
                article_data = filename_to_article[fname]
                gt_text, has_gt = get_ground_truth_text(article_data, self.dataset_type)
                enhanced_result['ground_truth'] = gt_text
                enhanced_result['has_ground_truth'] = has_gt
                # Attach canonical article_id for downstream alignment
                enhanced_result['article_id'] = filename_to_idx[fname]
                if has_gt:
                    is_correct = result.get('direction', '').lower() == gt_text
                    enhanced_result['correct'] = is_correct
                    if is_correct:
                        correct_count += 1
                    total_with_truth += 1
            else:
                # Filename not found in original list; preserve entry without ground truth
                enhanced_result['has_ground_truth'] = False
    ...
    def _save_article_results(self, article_id: int, llama32_res: Dict, qwen3_res: Dict, 
        ...
        with open(article_dir / "llama32_response.json", 'w', encoding='utf-8') as f:
            json.dump(llama32_res, f, indent=2, ensure_ascii=False)
        
        with open(article_dir / "qwen3_response.json", 'w', encoding='utf-8') as f:
            json.dump(qwen3_res, f, indent=2, ensure_ascii=False)
    ...
    def _save_session_summary(self, results: List[Dict]):
        ...
        summary = {
            'session_info': {
                ...
                'models_used': [
                    self.config['vllm']['small_ensemble']['llama32']['model_id'],
                    self.config['vllm']['small_ensemble']['qwen3']['model_id'],
                    self.config['vllm']['small_ensemble']['mistral']['model_id'],
                ]
            },
    ...
    def _validate_results(self, llama32_results: List, qwen3_results: List, mistral_results: List) -> bool:
        ...
        if len(llama32_results) != len(qwen3_results) or len(qwen3_results) != len(mistral_results):
            logger.error(f"Result count mismatch: Llama32={len(llama32_results)}, "
                        f"Qwen3={len(qwen3_results)}, Mistral={len(mistral_results)}")
            return False
        
        if len(llama32_results) == 0:
            logger.error("No articles were successfully processed")
            return False
        
        return True

    def _align_results_by_filename(
        self,
        llama32_results: List[Dict],
        qwen3_results: List[Dict],
        mistral_results: List[Dict],
        articles: List[tuple]
    ) -> Tuple[List[Dict], List[Dict], List[Dict]]:
        """Align per-model results by filename, keeping only articles present in all three.

        - Filters out any result whose filename is not in the original articles list
        - Intersects filenames across models
        - Orders aligned results by the original dataset order
        - Injects canonical article_id from the original dataset for downstream use
        """
        # Build canonical mapping from filename to index
        filename_to_idx: Dict[str, int] = {}
        for idx, (_, fname) in enumerate(articles):
            filename_to_idx[fname] = idx
        ...
        llama_map = to_map(llama32_results)
        qwen_map = to_map(qwen3_results)
        mistral_map = to_map(mistral_results)

        common_filenames = set(llama_map.keys()) & set(qwen_map.keys()) & set(mistral_map.keys())

        if not common_filenames:
            logger.error("No common filenames across models after alignment")
            return [], [], []

        # Order by original dataset index for stable downstream behavior
        ordered_common = sorted(common_filenames, key=lambda f: filename_to_idx[f])

        aligned_llama: List[Dict] = []
        aligned_qwen: List[Dict] = []
        aligned_mistral: List[Dict] = []

        for fname in ordered_common:
            idx = filename_to_idx[fname]
            l = llama_map[fname].copy()
            q = qwen_map[fname].copy()
            m = mistral_map[fname].copy()
            # Inject canonical article_id
            l['article_id'] = idx
            q['article_id'] = idx
            m['article_id'] = idx
            aligned_llama.append(l)
            aligned_qwen.append(q)
            aligned_mistral.append(m)

        dropped_counts = (
            len(llama32_results) - len(aligned_llama),
            len(qwen3_results) - len(aligned_qwen),
            len(mistral_results) - len(aligned_mistral),
        )
        logger.info(
            f"Aligned results by filename: kept {len(ordered_common)} articles; "
            f"dropped (llama32={dropped_counts[0]}, qwen3={dropped_counts[1]}, mistral={dropped_counts[2]})"
        )

        return aligned_llama, aligned_qwen, aligned_mistral
...
async def main(data_dir: str = None, dataset: str = None, n_samples: int = 1000, use_original: bool = False,
    ...
    if data_dir:
        ...
        if dataset.lower() == 'baly':
            from src.models.llama32_labeler import load_random_articles
            ...
            from src.models.llama32_labeler import load_budak_articles
        ...
        if not dataset:
            ...
            logger.error("\nExamples:")
            logger.error("  python ensemble_multi_model_small.py --dataset baly")
            logger.error("  python ensemble_multi_model_small.py --dataset ad_fontes")
            logger.error("  python ensemble_multi_model_small.py --dataset budak")
        ...
        if balanced_dataset_path:
            ...
            logger.error(f"  python ensemble_multi_model_small.py --use-original --dataset {dataset_type} --n_samples {n_samples}")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description='Multi-model ensemble bias detection with balanced dataset support (Small Models)',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # Use balanced dataset for baly
  python ensemble_multi_model_small.py --dataset baly
  
  # Use balanced dataset for ad_fontes
  python ensemble_multi_model_small.py --dataset ad_fontes
  
  # Use specific balanced dataset directory
  python ensemble_multi_model_small.py --data-dir data/balanced_datasets/balanced_baly
  
  # Use original unbalanced data
  python ensemble_multi_model_small.py --use-original --dataset baly --n_samples 500
        """
    )
