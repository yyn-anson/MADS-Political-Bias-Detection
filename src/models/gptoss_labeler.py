"""
GPT-OSS-20B political bias labeler backed by a vLLM server.
...
# GPT-OSS uses the "Reasoning: medium" directive to enable its chain-of-thought mode.
_USER_TEMPLATE = """\
Reasoning: medium
...
class GPTOSSLabeler(BaseLabeler):
    ...
    GPT-OSS supports a "Reasoning: medium" directive in the prompt to activate
    its chain-of-thought reasoning mode, which is included in the user template.
    ...
    def load_model(self) -> None:
        ...
        try:
            ...
            self._client = None
            raise RuntimeError(
                f"GPT-OSS vLLM server at {self._base_url} is not reachable: {exc}"
            ) from exc
        logger.info(f"Connected to GPT-OSS vLLM server at {self._base_url} (model: {self.model_name})")
    ...
    def predict(self, article_text: str) -> Dict[str, Any]:
        ...
        try:
            ...
            raise RuntimeError(f"GPT-OSS vLLM API call failed: {exc}") from exc
    ...
    def generate_discussion_challenge(
        ...
        try:
            ...
            raise RuntimeError(f"GPT-OSS challenge API call failed: {exc}") from exc
    ...
    def generate_discussion_response(
        ...
        try:
            ...
            raise RuntimeError(f"GPT-OSS response API call failed: {exc}") from exc
