"""
Llama-3.2 political bias labeler backed by a vLLM server.
...
class LlamaLabeler(BaseLabeler):
    ...
    def load_model(self) -> None:
        ...
        try:
            ...
            self._client = None
            raise RuntimeError(
                f"Llama vLLM server at {self._base_url} is not reachable: {exc}"
            ) from exc
        logger.info(f"Connected to Llama vLLM server at {self._base_url} (model: {self.model_name})")
    ...
    def predict(self, article_text: str) -> Dict[str, Any]:
        ...
        try:
            ...
            raise RuntimeError(f"Llama vLLM API call failed: {exc}") from exc
    ...
    def generate_discussion_challenge(
        ...
        try:
            ...
            raise RuntimeError(f"Llama challenge API call failed: {exc}") from exc
    ...
    def generate_discussion_response(
        ...
        try:
            ...
            raise RuntimeError(f"Llama response API call failed: {exc}") from exc
