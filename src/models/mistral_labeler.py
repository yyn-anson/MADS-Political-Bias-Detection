"""
Mistral political bias labeler backed by a vLLM server.
...
class MistralLabeler(BaseLabeler):
    """Political bias labeler using Mistral served by vLLM.

    The vLLM server must be running before instantiating this class.
    Example (small ensemble, 7B):
        vllm serve mistralai/Mistral-7B-Instruct-v0.3 --port 8003 --api-key token-abc123
    Example (regular ensemble, 22B):
        vllm serve mistralai/Mistral-Small-Instruct-2409 --port 8003 --api-key token-abc123
    """
    ...
    def load_model(self) -> None:
        ...
        try:
            ...
            self._client = None
            raise RuntimeError(
                f"Mistral vLLM server at {self._base_url} is not reachable: {exc}"
            ) from exc
        logger.info(f"Connected to Mistral vLLM server at {self._base_url} (model: {self.model_name})")
    ...
    def predict(self, article_text: str) -> Dict[str, Any]:
        ...
        try:
            ...
            raise RuntimeError(f"Mistral vLLM API call failed: {exc}") from exc
    ...
    def generate_discussion_challenge(
        ...
        try:
            ...
            raise RuntimeError(f"Mistral challenge API call failed: {exc}") from exc
    ...
    def generate_discussion_response(
        ...
        try:
            ...
            raise RuntimeError(f"Mistral response API call failed: {exc}") from exc
